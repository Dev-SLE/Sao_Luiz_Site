            ...
            AND mc.created_at > COALESCE(
              (
                ...
                WHERE ma.conversation_id = c.id
                  AND ma.sender_type IN ('AGENTE', 'IA')
              ),
    ...
    if (status) {
      const targetStageRes = await pool.query(
        `
          ...
          WHERE l.id = $1::uuid
            AND (
              ($2 = 'CONCLUIDO' AND LOWER(s.name) IN ('concluído', 'concluido', 'finalizado'))
              OR ($2 = 'EM_RASTREIO' AND LOWER(s.name) IN ('em rastreio', 'rastreio'))
