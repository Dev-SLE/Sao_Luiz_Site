...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (file.sharepoint_item_id && file.sharepoint_site_id && file.sharepoint_drive_id) {
      ...
      await recordFileAccess(pool, file.id, "download", session?.username || null);
      return new NextResponse(res.body, {
        status: 200,
        headers: {
          "Content-Type": ct,
          "Cache-Control": "private, max-age=60",
          "Content-Disposition": `attachment; filename="${encodeURIComponent(file.original_name || file.file_name)}"`,
        },
      });
    }
