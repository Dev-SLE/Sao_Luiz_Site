...
import { selectFunilEvolucaoMensal } from "@/lib/server/biFunilVendasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { rows, meta } = await selectFunilEvolucaoMensal(pool, url);
    return NextResponse.json({ rows, periodApplied: meta.periodApplied });
  } catch (error) {
    console.error("GET /api/bi/funil-vendas/evolucao-mensal:", error);
