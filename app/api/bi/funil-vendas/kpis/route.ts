...
import { selectFunilKpis } from "@/lib/server/biFunilVendasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { rows, meta } = await selectFunilKpis(pool, url);
    return NextResponse.json({ rows, periodApplied: meta.periodApplied });
  } catch (error) {
    console.error("GET /api/bi/funil-vendas/kpis:", error);
    return NextResponse.json({ error: "Falha ao carregar indicadores do funil de vendas." }, { status: 500 });
  }
}
