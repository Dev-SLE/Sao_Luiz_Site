...
import { selectFunilQtdFechadaVendedor } from "@/lib/server/biFunilVendasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { rows, meta } = await selectFunilQtdFechadaVendedor(pool, url);
    return NextResponse.json({ rows, periodApplied: meta.periodApplied });
  } catch (error) {
    console.error("GET /api/bi/funil-vendas/quantidade-fechada-vendedor:", error);
    return NextResponse.json({ error: "Falha ao carregar quantidade fechada por vendedor." }, { status: 500 });
  }
}
