...
import { selectFunilFunilStatus } from "@/lib/server/biFunilVendasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { rows, meta } = await selectFunilFunilStatus(pool, url);
    return NextResponse.json({ rows, periodApplied: meta.periodApplied });
  } catch (error) {
    console.error("GET /api/bi/funil-vendas/funnel:", error);
    return NextResponse.json({ error: "Falha ao carregar funil por status." }, { status: 500 });
  }
}
