...
import { selectFluxoFacetOptions } from "@/lib/server/biFluxoRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const data = await selectFluxoFacetOptions(pool, url);
    return NextResponse.json(data, {
      headers: { "Cache-Control": "no-store, max-age=0" },
    });
  } catch (error) {
    console.error("GET /api/bi/fluxo/facet-options:", error);
