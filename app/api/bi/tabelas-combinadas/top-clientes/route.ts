...
import { resolveTcPeriod, selectTcTopClientes } from "@/lib/server/biTabelasCombinadasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = await selectTcTopClientes(pool, url);
    return NextResponse.json({ rows, periodApplied: resolveTcPeriod(url) });
  } catch (error) {
    console.error("GET /api/bi/tabelas-combinadas/top-clientes:", error);
    return NextResponse.json({ error: "Falha ao carregar clientes prioritarios." }, { status: 500 });
  }
}
