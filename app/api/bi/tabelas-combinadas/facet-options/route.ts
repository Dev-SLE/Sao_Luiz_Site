...
import { selectTcFacetOptions } from "@/lib/server/biTabelasCombinadasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const data = await selectTcFacetOptions(pool, url);
    return NextResponse.json(data);
  } catch (error) {
    console.error("GET /api/bi/tabelas-combinadas/facet-options:", error);
