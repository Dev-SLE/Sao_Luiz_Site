...
import { resolveTaxasPeriod, selectTaxasTable } from "@/lib/server/biTaxasRead";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { rows, meta } = await selectTaxasTable(pool, url);
    return NextResponse.json({ rows, meta, periodApplied: resolveTaxasPeriod(url) });
  } catch (error) {
    console.error("GET /api/bi/taxas/table:", error);
