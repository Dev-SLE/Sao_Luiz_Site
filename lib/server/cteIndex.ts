    ...
    computed AS (
      SELECT
        ...
        CASE
          ...
          WHEN lp.process_status = 'OCORRENCIA' THEN 'ocorrencias'
          WHEN lp.process_status = 'TAD' THEN 'ocorrencias'
          WHEN lp.process_status = 'EM BUSCA' AND UPPER(COALESCE(lp.process_description, '')) LIKE '%TAD%' THEN 'ocorrencias'
    ...
    computed AS (
      SELECT
        ...
        CASE
          ...
          WHEN ${normalizedStatusExpr("c.status")} LIKE 'CONCLUIDO%' THEN 'concluidos'
          WHEN ${normalizedStatusExpr("c.status")} LIKE 'ENTREGUE%' THEN 'concluidos'
          WHEN (SELECT process_status FROM latest_process) IN ('TAD', 'OCORRENCIA') THEN 'ocorrencias'
